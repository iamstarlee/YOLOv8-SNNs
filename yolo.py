...
class YOLO(object):
    ...
    def detect_image_pose(self, image, crop = False, count = False):
        ...
        with torch.no_grad():
            ...
            for pred, orig_img, img_path in zip(result, [img], dummy_input):
                print(f"pred is {pred.shape}")
                pred[:, :4] = ops.scale_boxes((640, 320), pred[:, :4], orig_img.shape).round()
                pred_kpts = pred[:, 6:].view(1, 17, 3) if len(pred) else pred[:, 6:]
                pred_kpts = ops.scale_coords((640, 320), pred_kpts, orig_img.shape)
                results.append(
                    Results(orig_img, path=img_path, names="yolov8s-pose", boxes=pred[:, :6], keypoints=pred_kpts)
                )
            
            # print(f"img is {img.transpose(2, 0, 1).shape}")
            for r in results:
                plotted_img = r.plot(
                    line_width = None,
                    boxes = True,
                    conf = True,
                    labels = True,
                    im_gpu = img.transpose(2, 0, 1)
                )
                outimg_path = 'imgs_out/'
                
                success = cv2.imwrite(outimg_path + 'saved.png', plotted_img)
                if success:
